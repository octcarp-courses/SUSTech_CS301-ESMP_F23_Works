	...
	for(h=0;h<row;h++) {
		for(m=0;m<column;m++) {
            if(h>=pa_y && h<pa_y+pa_h && m>=pa_x && m<pa_x+pa_w){
			    LCD_Fast_DrawPoint(pos_x+m-pa_x,pos_y+h-pa_y,*data);
            }
            *data++;
		}
	}
    LCD_DrawRectangle(pos_x, pos_y, pos_x+ pa_w, pos_y+pa_h);
}
...
uint16_t pa_x,uint16_t pa_y, uint16_t pa_w,uint16_t pa_h,unsigned short *pic)
{
	...
	for(h=0;h<row;h++) {
		for(m=0;m<column;m++) {
            if(h>=pa_y && h<pa_y+pa_h && m>=pa_x && m<pa_x+pa_w){
			    LCD_Fast_DrawPoint(x+m,y+h,WHITE);
            }else{
			    LCD_Fast_DrawPoint(x+m,y+h,*data);
            }
            *data++;
		}
	}
